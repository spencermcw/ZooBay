...
async function main() {
    ...
    await hegg.connect(signer).mint(runs);
    console.log('minting anml')
    await anml.connect(signer).mint(runs);
    console.log('minting hanml')
    await hanml.connect(signer).mint(runs);

    for (let i = 0; i < runs; i++) {
        console.log('Creating Listing', i);
        await auctionHouse.connect(signer).createListing(
            [egg.address, hegg.address, anml.address, hanml.address],
            [[i], [i], [i], [i]],
            "1",
            "0",
